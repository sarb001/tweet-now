...
export const LoginUser = createAsyncThunk('/api/v1/login',async(userData,{rejectWithValue }) => {
    try {
        ...
        const response = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/v1/login`,userData ,{
            withCredentials : true,
        });
...
export const UserProfile = createAsyncThunk('/api/v1/profile' ,async (userData , {rejectWithValue }) => {
    try {   
        const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/v1/profile` , {
            headers :{
                'Content-Type' : 'application/json',
            },
            withCredentials : true,
